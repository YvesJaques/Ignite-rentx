...
let CreateRentalUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (carUnavailable) {
      throw new _AppError.AppError("Car is not available");
    }

    const rentalOpenToUser = await this.rentalsRepository.findOpenRentalByUser(user_id);

    if (rentalOpenToUser) {
      throw new _AppError.AppError("There's a rental in progress for this user!");
    }
    ...
    if (compare < this.minimumRentalHours) {
      throw new _AppError.AppError(`Can't register a rental with less than ${this.minimumRentalHours} hours return time.`);
    }
